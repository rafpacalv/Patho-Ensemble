#!/usr/bin/env python3
"""
Late fusion con optimización de pesos vía CMA-ES.

Mismo problema que `ensemble_nelder_mead.py` — combinación lineal de
predicciones de varios modelos, P_final = Σ w_i * P_i, con pesos optimizados
para maximizar AUC in-sample (train_eval) y evaluados en test — pero con un
optimizador poblacional (Hansen, CMA-ES) en vez de un simplex local
(Nelder-Mead). Es el optimizador que usa el post-hoc ensembling de
Auto-Sklearn 2.0 (Purucker & Beel, 2023, arXiv:2307.00286) para construir
ensembles a partir de una librería de modelos, precisamente para objetivos
ruidosos y no convexos como el AUC sobre pocas muestras — el mismo régimen en
el que Nelder-Mead ya dio +0.0052, por debajo del suelo de reproducibilidad
del proyecto (≈0.004).

No usa el paquete `cma` para model selection (qué modelos entran), sólo para
el sub-problema de pesos: la elección de modelos ya se investiga aparte en
`sweep_base_models.py`.

Uso: idéntico a ensemble_nelder_mead.py, mismos flags de datos y folds.
"""
...
from utils import Metrics  # noqa: F401 (paridad con ensemble_nelder_mead.py)
...
def load_fold_predictions(work_dir, train_source, tissue_patching, task_name,
    ...
    Retorna (preds, labels); (None, None) si el fold no existe.
    """
    if split == "val":
...
def optimize_fold_weights_cmaes(P_train_dict, y_train, P_test_dict, y_test,
                                model_names, fold_k, sigma0=0.5, popsize=None,
                                maxfevals=2000, seed=0):
    """Optimiza pesos per-modelo vía CMA-ES en train_eval, evalúa en test.

    Misma parametrización que Nelder-Mead: pesos = softmax(w_raw), para que
    la comparación entre optimizadores no esté confundida por la
    parametrización del espacio de búsqueda.

    Returns
    -------
    w_opt, auc_test, preds_test, auc_train, n_evals
    """
    n = len(model_names)
    ...
    def objective(w_raw):
        ...
        return -roc_auc_score(y_train, P_cand)

    x0 = np.zeros(n)  # softmax(0,...,0) = pesos iguales, mismo punto de partida que Nelder-Mead
    opts = {"seed": seed, "verbose": -9, "maxfevals": maxfevals}
    if popsize:
        opts["popsize"] = popsize
    es = cma.CMAEvolutionStrategy(x0, sigma0, opts)
    es.optimize(objective)

    w_opt = np.exp(es.result.xbest) / np.exp(es.result.xbest).sum()
...
def main(work_dir, train_source, tissue_patching, task_name,
    ...
    print(f"\n{'='*80}")
    print(f"Opción B: Late Fusion + CMA-ES Weight Optimization")
    print(f"{'='*80}")
    ...
    print(f"sigma0={sigma0}, popsize={popsize or 'auto'}, maxfevals={maxfevals}")
    print(f"{'='*80}\n")

    results_subdir = f"{work_dir}/{train_source}/{task_name}/abmil/ensemble4_cmaes_{'_'.join(foundational_models)}"
    ...
    for fold_k in range(fold_start, min(fold_end + 1, n_folds)):
        ...
        w_opt, auc_test, preds_test, auc_train, n_evals = optimize_fold_weights_cmaes(
            P_train_dict, y_train, P_test_dict, y_test,
            foundational_models, fold_k,
            sigma0=sigma0, popsize=popsize, maxfevals=maxfevals, seed=seed + fold_k)
        ...
        print(f"  CMA-ES evaluations: {n_evals}")
    ...
    print(f"\n{'='*80}")
    print(f"✓ CMA-ES optimization complete ({len(all_fold_metrics)} folds)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sigma0", type=float, default=0.5,
                        help="paso inicial de CMA-ES (misma escala que los w_raw pre-softmax)")
    parser.add_argument("--popsize", type=int, default=None,
                        help="tamaño de población; por defecto el heurístico de CMA-ES (4+3ln(n))")
    parser.add_argument("--maxfevals", type=int, default=2000,
                        help="máximo de evaluaciones de la función objetivo por fold")
